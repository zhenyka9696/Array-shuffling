...
function Main(){
  ...
  var res=[];
  var input=0;
  //alert('Hello, are we calculating our values or yours, today?'); mocha не воспринимает alert
  console.log('Hello, are we calculating our values or yours, today?');
  ...
  function addValue() {
    ...
    for(var i=0; i >= 0; i++) {
        ...
        if(deck[i] === null||deck[i]=='exit') { // если  нажимаем "Отмена"//
            //alert('Отмена');
            console.log('Отмена');
            console.log('deck before maping= '+deck);
            //if (typeof deck!=)
            deck.pop();//Убираем нуль из массива
            //deck=deck.map(Number);//If we wanna make this pretty,but,unfortunately, it only working with numbers
